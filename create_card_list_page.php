...
echo "<!DOCTYPE html>";
echo "<html lang='ja'>";
echo "<head><meta charset='UTF-8'><title>カード型一覧ページ作成</title></head>";
echo "<body>";
echo "<h1>🎴 カード型一覧ページ作成</h1>";

// 新しいlist.phpの内容
$new_list_content = '<?php
// データベース接続
...
if (!empty($search)) {
    $sql .= " AND (ai_service LIKE ? OR description LIKE ?)";
    $search_param = "%" . $search . "%";
    $params[] = $search_param;
    $params[] = $search_param;
    $types .= "ss";
    ...
    <div class="container-fluid px-4 py-3">
        ...
        <div class="search-section">
            ...
            <div class="mt-3">
                ...
                <a href="?search=&sort=<?php echo $sort; ?>&free=1" class="btn btn-outline-success filter-btn">
                    <i class="fas fa-gift me-1"></i>無料
                </a>
                <a href="?search=&sort=<?php echo $sort; ?>&featured=1" class="btn btn-outline-warning filter-btn">
                    ...
                    <div class="col">
                        <div class="card ai-card h-100">
                            <div class="card-header card-header-custom d-flex align-items-center">
                                ...
                                <?php else: ?>
                                    <div class="ai-icon me-3 bg-secondary d-flex align-items-center justify-content-center text-white">
...
// 既存のlist.phpをバックアップ
$list_file = 'list.php';
$backup_file = 'list_backup_card_' . date('Ymd_His') . '.php';

if (file_exists($list_file)) {
    if (copy($list_file, $backup_file)) {
        echo "<p style='color:green'>✓ 既存のlist.phpをバックアップしました: $backup_file</p>";
    }
}

// 新しいlist.phpを作成
if (file_put_contents($list_file, $new_list_content)) {
    echo "<p style='color:green'>✓ 新しいカード型list.phpを作成しました</p>";
    
    echo "<h3>✨ 新機能:</h3>";
    echo "<ul>";
    echo "<li>📱 <strong>レスポンシブカードレイアウト</strong> - モバイル、タブレット、デスクトップ対応</li>";
...
echo "<h2>🧪 テスト方法</h2>";
echo "<ol>";
echo "<li><a href='list.php' target='_blank'>新しいカード型一覧ページを確認</a></li>";
